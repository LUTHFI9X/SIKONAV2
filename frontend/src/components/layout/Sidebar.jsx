    ...
    <aside className={`sidebar z-[120] transition-transform duration-300 ease-out ${mobileOpen ? 'translate-x-0 shadow-2xl shadow-slate-950/45' : '-translate-x-full'} lg:translate-x-0 lg:shadow-none`}>
      ...
      <div className="p-5 mb-2 relative">
        <button
          ...
          className="lg:hidden absolute top-3 right-3 w-8 h-8 rounded-lg border border-violet-300/25 bg-violet-500/10 text-violet-100 hover:bg-violet-500/20 flex items-center justify-center"
        >
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-violet-300/70 text-[10px] font-semibold tracking-wider mt-0.5">Sistem Konsultasi Audit</p>
          </div>
        </div>
        {/* Separator line with glow */}
        <div className="mt-5 h-px bg-gradient-to-r from-transparent via-violet-400/30 to-transparent"></div>
      </div>
      ...
      <nav className="flex-1 px-3 overflow-y-auto scrollbar-thin">
        {mainMenuItems.map((item, index) => (
          <div key={item.path}>
            {index === 1 && (
              <div className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-violet-300/45 select-none flex items-center gap-2">
                <span className="flex-1 h-px bg-gradient-to-r from-violet-400/20 to-transparent"></span>
                <span>{t('menu.mainMenu', 'Menu Utama')}</span>
                <span className="flex-1 h-px bg-gradient-to-l from-violet-400/20 to-transparent"></span>
              </div>
            )}
            <NavLink
              to={item.path}
      ...
      {/* Bottom Section */}
      <div className="p-4 border-t border-violet-400/10 space-y-1 relative">
        {/* Top glow line */}
        <div className="absolute top-0 left-4 right-4 h-px bg-gradient-to-r from-transparent via-violet-400/30 to-transparent"></div>
        
        {(currentUser?.role === 'auditor' || currentUser?.role === 'admin' || (currentUser?.role === 'manajemen' && currentUser?.sub_role === 'admin')) && (
          <NavLink 
            to="/profile" 
            onClick={closeMobileSidebar}
            className={({ isActive }) => `flex items-center gap-3 px-4 py-3 text-violet-200 hover:bg-violet-500/10 hover:text-violet-50 rounded-xl cursor-pointer transition-all duration-300 font-semibold text-sm group ${isActive ? 'bg-violet-500/15 text-violet-50' : ''}`}
          >
